...
*/
template<typename T>
void sort(jax::Unity<T>* array){
  if(array == nullptr){
    throw jax::UnityException("passed array == nullptr in sort(Unity<T>* array)");
  }
  jax::MemoryState origin = array->getMemoryState();
  jax::MemoryState fore = array->getFore();
  if(origin == jax::cpu || fore == jax::cpu) array->transferMemoryTo(jax::gpu);
  thrust::device_ptr<T> array_ptr(array->device);
  thrust::device_ptr<T> new_end = thrust::sort(array_ptr,array_ptr+array->size());
  CudaCheckError();
  array->setFore(jax::gpu);
  if(origin != jax::gpu) array->setMemoryState(origin);
}
/**
* \ingroup cuda_util
* \see Unity
...
*/
template<typename T>
void sort(jax::Unity<T>* array, bool (*compare)(const T&,const T&)){
  if(array == nullptr){
    throw jax::UnityException("passed array == nullptr in sort(Unity<T>* array)");
  }
  jax::MemoryState origin = array->getMemoryState();
...
*/
template<typename T>
void remove(jax::Unity<T>* array, bool (*validate)(const T&)){
  if(array == nullptr){
    throw jax::UnityException("passed array == nullptr in remove(Unity<T>* array)");
  }
  jax::MemoryState origin = array->getMemoryState();
  jax::MemoryState fore = array->getFore();
  if(origin == jax::cpu || fore == jax::cpu) array->transferMemoryTo(jax::gpu);
  ...
  if(numElements == 0){
    ...
    array->setData(array_device,numElements,jax::gpu);
  }
  if(origin != jax::gpu) array->setMemoryState(origin);
}
